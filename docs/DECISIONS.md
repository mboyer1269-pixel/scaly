...
**Contexte.** P4 liste « RLS Postgres ». Or l'app est mono-tenant de fait : toutes les routes utilisent DEFAULT_COMPANY_ID, une seule connexion Prisma avec droits complets, et le rattachement tenant↔session n'existe pas encore (ADR-013). Activer la Row Level Security maintenant exigerait de poser `SET app.current_company` par requête (plomberie Prisma par middleware/`$extends`) alors qu'AUCUNE requête ne porte d'identité de tenant — des politiques RLS seraient du théâtre de sécurité : elles laisseraient passer tout ce que l'app fait déjà.
**Décision.** RLS différé : il sera activé EN MÊME TEMPS que le rattachement Clerk org ↔ company (premier client réel multi-tenant), avec politiques par companyId + rôle applicatif non-propriétaire de schéma + `SET LOCAL app.current_company` injecté par un client Prisma étendu. En P4, le durcissement proportionné livré est : rate limiting fenêtre glissante sur les routes coûteuses (/api/onboarding/draft 5/10 min, /api/billing/checkout 10/10 min, /api/simulate 30/10 min — par instance, documenté tel quel), secrets vérifiés sur tous les webhooks/crons (signature Twilio, HMAC Stripe, Bearer CRON_SECRET), champs protégés côté serveur (planId/billing inaccessibles au PUT /api/company).
**Conséquences.** + Pas de fausse assurance : la posture réelle est documentée. + La liste exacte des prérequis RLS est écrite pour le jour du multi-tenant. − Jusque-là, la défense de la base repose sur l'app et la connexion unique — acceptable tant qu'il n'y a qu'un tenant pilote.
...
**Décision.** Toute route/page DE SESSION résout son tenant via `src/server/tenant.ts` : `resolveCompanyId()` lit le claim `metadata.companyId` du session token Clerk (même mécanisme que le rôle, ADR-013 — poser `publicMetadata.companyId` sur l''utilisateur dans le dashboard Clerk) ; sans auth active, sans session ou sans claim → tenant démo, honnêtement. Les canaux SANS session ne passent jamais par là : le webhook Stripe résout par `metadata.companyId`/`client_reference_id` posés au checkout (repli démo) ; les webhooks Twilio voix/SMS restent sur le tenant démo jusqu''au mapping numéro de téléphone → company (prérequis : un numéro par client) ; les crons itèrent toutes les compagnies. Gardes d''appartenance anti-IDOR ajoutées sur GET /api/calls/:id, POST /api/calls/:id/analyze et la page détail d''appel (introuvable si autre tenant, sauf rôle founder) ; `?companyId=` de /api/calls réservé au founder.
**Conséquences.** + Le multi-tenant devient un branchement (poser le claim + seeder la company), plus une réécriture. + Les fuites cross-tenant par IDOR sont fermées AVANT d''avoir deux tenants. − Le tenant démo reste le repli universel : acceptable en pilote, à transformer en erreur explicite quand l''onboarding self-serve créera les compagnies. − Restant connu : voice-lab save/sessions et /api/export ne vérifient pas l''appartenance au-delà du tenant résolu — à reprendre avec le mapping téléphonie.
...
**Contexte.** Le repo a dépassé P0 : Prisma, Clerk, moteur LLM, Voice Lab, pont temps réel, actions SMS, Stripe et mémoire inter-appels existent. Le risque n'est plus l'absence de features mais la DÉRIVE entre ce que le code fait et ce que docs/README/ROADMAP/`/status` affirment — et l'absence d'une réponse vérifiable à « peut-on faire un premier appel réel aujourd'hui, et qu'est-ce qui est bloquant ? ». Deux pièges précis menaçaient le premier pilote : (a) `resolveCompanyId()` retombait SILENCIEUSEMENT sur `DEFAULT_COMPANY_ID` quand l'auth est active mais qu'un utilisateur connecté n'a pas de claim `companyId` — un non-founder voyait alors les données du tenant démo ; (b) le webhook voix `/api/voice/incoming` résout `DEFAULT_COMPANY_ID` en dur (pas de session), donc accueillir un 2ᵉ client croiserait les tenants.
**Décision.** (1) **Pilot Readiness Gate** : `evaluatePilotReadiness()` PUR et testé (`src/services/pilot-readiness.ts` + `tests/pilot-readiness.test.ts`), exposé par `npm run pilot:check` (CLI `scripts/pilot-check.ts`) et joué en CI. Verdict PASS/WARN/FAIL sur : auth (ou mode dev assumé), cohérence `STORE_PROVIDER`/`DATABASE_URL` + aller-retour Prisma live, `REALTIME_SHARED_SECRET` en prod, `TWILIO_AUTH_TOKEN` si webhook public, transport realtime + repli `<Dial>`, `OPENAI_API_KEY`, posture tenant, commandes CI, tests garde-fous, purge Loi 25. FAIL → code de sortie 1 ; WARN (démo, mono-tenant) → 0. (2) **Repli de tenant signalé** : `decideTenant()` pur distingue `claim` / `demo-fallback` / `no-auth` et lève `warn=true` pour un non-founder sans claim (le founder utilise la démo par conception, ADR-017) ; `resolveTenant()` émet alors un `console.warn`. `resolveCompanyId()` garde sa signature — zéro changement d'appelant. (3) **Bloquant documenté** : TODO dur dans `/api/voice/incoming` + flag `twilioTenantMapping:false` du gate → le rapport affiche « MONO-TENANT PILOTE UNIQUEMENT » tant que le mapping numéro→companyId n'existe pas. (4) **Mémoire inter-appels** : `selectCallerHistory()` pur ferme les deux fuites (numéro masqué → aucune mémoire ; anti-croisement de tenant) et est testé.
**Conséquences.** + Une seule commande répond aux 7 questions du pilote, sans complaisance, et la CI empêche la régression du contrat (commandes + tests garde-fous). + Le repli démo cesse d'être invisible. + La règle « mono-tenant tant que le mapping n'existe pas » est mécaniquement affichée, pas seulement écrite. + README/ROADMAP/VOICE/`/status` réalignés sur la table de vérité. − Le gate lit la CONFIGURATION, pas le comportement réseau réel : un appel Twilio bout-en-bout, les latences réelles et l'aller-retour Postgres live restent à vérifier à la main (le gate les marque « non vérifié » plutôt que prêts). − Le warn de tenant est un log serveur, pas un blocage dur : suffisant en pilote mono-tenant, à transformer en erreur explicite quand l'onboarding self-serve créera les compagnies.
