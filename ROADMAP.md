...
**P2B — Transport réel 🔧 (repli humain réel vérifié, IA realtime restante)** : webhook Twilio signé + mapping numéro appelé → tenant + repli `<Dial>` (le téléphone ne casse jamais), appel réel persisté `source:"live"` et preuve `live_call` vérifiée le 2026-06-30. Le pont scaly-realtime est codé (Media Streams ↔ OpenAI Realtime, µ-law passthrough, barge-in, transfert humain par outil, latences RÉELLES mesurées par tour), prompt système testé (divulgation IA, consentement de rappel ADR-015), fin d'appel → Call `source:"live"` analysé par le moteur existant. **Restants** : brancher `SCALY_REALTIME_WS_URL`, puis jalons (4) 50 appels tests FR/EN dont urgences et (5) latence perçue < 800 ms p50 / < 1200 ms p95 — MESURÉE.
